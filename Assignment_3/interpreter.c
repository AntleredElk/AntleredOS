...
{
    size_t num_tokens = 1;
    int flag = 0;   // Flag to track spaces
...
int exec(const char *path_1, const char *path_2, const char *path_3){
    ...
    if(path_3 != NULL){
        ...
        else if(!strcmp(path_1, path_3)) {
            printf("Error: Script %s already loaded\n", path_1);
            return 1;
        }
        else if(!strcmp(path_2, path_3)) {
            printf("Error: Script %s already loaded\n", path_2);
            return 1;
        }
        else{
...
int interpret(char *raw_input)
{
    ...
    {
        if (!(tokens[1] != NULL))  // limits execution to 3 concurrent programs
        {
            printf("exec: Malformed command\n");
            free(tokens);
            return 1;
        }
        exec(tokens[1], tokens[2], tokens[3]);
        free(tokens);
        return 0;
    }
